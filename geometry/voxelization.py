...
class VoxelizedMesh:
    ...
    def __init__(self) -> None:
        ...
        # bookkeeping the underlying mesh, could be None
        self.m_mesh: ModelData = None

        # 3d voxel grid
        self.m_voxdata: VoxelGrid = None
        self.m_shape: np.ndarray = None  # shape of the voxel grid at direction 0

        # padding size of the voxel grid, along each axis along both sides
        self.m_pad_size: tuple[int, int, int] = (0, 0, 0)
        self.m_pad_value: bool = False  # padding value of the voxel grid
    ...
    def from_model_data(
        ...
        out.m_mesh = modeldata
        out.m_pad_size = pad_size
        out.m_pad_value = pad_value
        out.m_unit = vox_unit

        out.m_box_local = Box_3.create_as_bounding_box_of_points(mesh.vertices)
        if pad_size is not None:
            new_box_lengths = out.m_box_local.lengths + np.array(pad_size) * 2 * vox_unit
            out.m_box_local.set_lengths(new_box_lengths[0], new_box_lengths[1], new_box_lengths[2])

        # adjust box size due to padding
    ...
    def from_trimesh(
        ...
        use_cache: bool = False,
        pad_size: tuple[int, int, int] | None = None,
        pad_value: int | float | bool | None = None,
    ) -> "VoxelizedMesh":
        """create a voxelized object from trimesh"""
        model = ModelData.from_trimesh(mesh)
        return cls.from_model_data(
            modeldata=model,
            vox_unit=vox_unit,
            vox_method=vox_method,
            use_cache=use_cache,
            pad_size=pad_size,
            pad_value=pad_value,
        )
    ...
    def _set_voxel_data(self, vox: VoxelGrid):
        self.m_voxdata = vox
        self.m_shape = np.array(vox.shape)

    def get_mesh_in_voxel_space(self) -> ModelData | None:
        """get the mesh in voxel space, only applicable when the mesh field is not None"""
        if self.m_mesh is None:
            return None
        output: ModelData = copy.deepcopy(self.m_mesh)
        output.set_transmat(self.transmat_local_to_voxel)
        return output
    ...
    def get_dense_voxel(self) -> np.ndarray:
        ...
        if vmat is None and self.m_voxdata is not None:
            ...
            if self.m_pad_size is not None:
                vmat = np.pad(
                    vmat,
                    list(zip(self.m_pad_size, self.m_pad_size)),
                    mode="constant",
                    constant_values=self.m_pad_value,
                )
        return vmat
    ...
    def clone(self, copy_voxel_data: bool = True, copy_cache: bool = True) -> "VoxelizedMesh":
        """clone the object
    ...
    def get_placement_transmat_other_to_this_voxel(
        self, other: "VoxelizedMesh", this_voxel_position: np.ndarray, other_pivot: np.ndarray = (0, 0, 0)
    ) -> np.ndarray:
    ...
    def get_placement_transmat_other_to_this_local(
        self, other: "VoxelizedMesh", this_voxel_position: np.ndarray, other_pivot: np.ndarray = (0, 0, 0)
    ) -> np.ndarray:
...
class MeshVoxelizer:
    ...
    @property
    def mesh(self) -> ModelData:
        """get mesh as ModelData

        returns
        --------
        ModelData
            The mesh as ModelData
        """
        return self.m_mesh
    ...
    def process(self, method: str = None):
        """run voxelization algorithm to voxelize the mesh in world space,
        where the contents inside mesh world-space bounding box are voxelized"""
        ...
        if method == VoxelizeMethod.trimesh:
            ...
            vox_indices = igfast.voxelize_mesh_to_grid(tmesh.vertices, tmesh.faces, self.m_voxel_unit, precision)
