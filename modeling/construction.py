...
def make_cube(size: float | np.ndarray, center: np.ndarray = None) -> tuple[np.ndarray, np.ndarray]:
    ...
    center
        ...
        is at the origin.
    size
        the edge length of the cube, can be a scalar or a 3-element array
    ...
    # if size is a scalar, make it a 3-element array
    assert size.size == 1 or size.size == 3, "size must be a scalar or a 3-element array"
    if size.size == 1:
        size = np.full(3, size)
